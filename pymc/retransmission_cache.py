...
class RetransmissionCache(object):
    def __init__(self, sender: 'ConnectionSender'):
        ...
        self._clean_cache_task = CleanRetransmissionQueueTask(self._connection.mConnectionId)
    ...
    def retransmit(self, low_seqno: int, high_seqno: int):
        ...
        if self._connection.isLogFlagSet(DistributorLogFlags.LOG_RETRANSMISSION_EVENTS):
            if self._queue.isEmpty():
                ...
                _first: RetransQueItm = self._queue.peekFirst()
                _last: RetransQueItm = self._queue.peekLast()
            ...
            if self._queue.isEmpty():
                ...
                if low_seqno < _first.seqno:
                    tNAKSequenceNumbers = self.getRetransmissionNAKSequenceNumbers(low_seqno, _first.seqno)
